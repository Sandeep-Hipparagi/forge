...
| `FR-004` | SHOULD | Accept an optional PRD (Markdown or plain text, ≤ 200 KB) that informs Planner scope. _(Brief: Good to Have)_   | With a PRD supplied, `TestPlan.sourceRefs[]` cites at least one PRD section id.                                                                             |
| `FR-005` | SHOULD | Accept optional natural-language intent, e.g. _"focus on checkout and authentication"_. _(Brief: Good to Have)_ | The named capabilities rank in the top 3 of the backlog when present in the map.                                                                            |
...
**On `FR-001`.** This requirement is why the API takes a session object rather than positional arguments: it lets every later capability arrive as an optional field without changing the required surface. The brief's _"sole required input"_ is a contract we can point at in code, not a claim in a README.
...
| `FR-105` | MUST   | Cluster raw states into named **capabilities** — user-meaningful units such as _"Checkout"_, not routes.                                | Every capability has a name, a description, an entry state, and ≥ 1 exit condition.                                                                                   |
...
**On `FR-106`.** Exploring somebody's live application means clicking things. A crawler that finds the delete button and presses it is not a testing tool, it is an incident. Read-only by default is a safety property; the demo makes it visible by showing what the Explorer _declined_ to press.
...
| `FR-207` | SHOULD | Reflect PRD content when supplied, citing the section behind each derived scenario. _(Brief: Good to Have)_                                           | `FR-004` acceptance, plus ≥ 1 scenario existing _only_ because of the PRD.                                                    |
| `FR-208` | SHOULD | Reflect natural-language intent in scenario selection and ordering. _(Brief: Good to Have)_                                                           | `FR-005` acceptance.                                                                                                          |
...
| `FR-307` | SHOULD | When a PRD is supplied, diff the plan against stated requirements and surface uncovered ones. _(Brief: Bonus)_                | `CoverageAssessment.prdGaps[]` names the uncovered requirement and cites its PRD section.                                 |
...
**Why this is the most valuable section in the document.** `FR-304` is the requirement that makes the orchestrator visibly _think_. Any team can chain three prompts. The observable moment where a plan is sent back with named deficiencies, and the second attempt is measurably better, is the demo beat that earns the 20% innovation weight.
...
| `FR-506` | SHOULD | Run independent capabilities in parallel with configurable worker count. _(Brief: Good to Have)_           | `workers: 4` reduces wall clock on a 4-capability suite by ≥ 40% versus serial, with identical verdicts. |
...
| `FR-606` | MUST   | State the _product-bug_ verdict as a defect report a developer can act on: what was expected, what happened, how to reproduce.            | `Diagnosis.defectReport` is non-null for `PRODUCT_BUG` and contains all three fields.          |
...
**On `FR-710`.** Retry without rollback is how automated repair corrupts a repository. Every patch is applied inside a transaction whose commit point is _verification passing_, not _the write succeeding_.
...
**On `FR-803`.** _"Your suite scores 34. Fix these four findings and it scores 71."_ That sentence is the entire business-impact criterion, and it is arithmetic we can show, not a claim we assert.
...
| `FR-904` | MUST   | Terminate in exactly one of: `COMPLETED`, `COMPLETED_PARTIAL`, `ESCALATED`, `ERROR`.                                                | Exit code derives from the terminal state **and** `Session.defectsFound` ([04 §3.4](../02-architecture/04-system-architecture.md)): `COMPLETED`/`COMPLETED_PARTIAL` with zero defects → 0; `COMPLETED`/`COMPLETED_PARTIAL` with ≥ 1 `PRODUCT_BUG` → **1**; `ESCALATED` → 2; `ERROR` → 3. A found defect is `COMPLETED`, not `ERROR` — resolved as `W-5` ([00 §7](../00-work-plan.md)). |
