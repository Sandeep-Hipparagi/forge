...
| **Ph1** Spine — **complete** | 60 min | Schemas, store, FSM skeleton, `runAgentLoop()`, API + SSE, the eval harness                     | Exit gate passed 2026-09-05: persisted replayable stub session, 94 tests, reset in 0.331 s                                             |
...
**Schema freeze recorded 2026-09-05.** `packages/core/schema/` is the Phase 1 contract
baseline. Any later edit requires an explicit cross-layer migration of Zod, SQLite, fixtures,
API projections, and affected replay cases; no Phase 2 work may silently widen it.
